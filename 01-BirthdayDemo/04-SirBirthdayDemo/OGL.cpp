...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CHAR:
	{
		...
		case 'w':
			...
			{
				if (gfCameraVertical <= 40.0f)
				gfCameraVertical = gfCameraVertical + 0.02f;
			}
			else
			{
				if (gfVertical <= 40.0f)
				{
					gfVertical = gfVertical + 0.02f;
				}
			}
		...
		case 'a':
			...
			if (switchCameraToMovement)
			{
				if (gfCameraHorizontal <= 40.0f)
				gfCameraHorizontal = gfCameraHorizontal + 0.02f;
			}
			else
			{
				if (gfHorizontal <= 40.0f)
				{
					gfHorizontal = gfHorizontal + 0.02f;
				}
			}
			break;
		case 'D':
		...
		case 'z':
			...
			if (switchCameraToMovement)
			{
				if (gfCameraDepth >= -40.0f)
				gfCameraDepth = gfCameraDepth - 0.02f;
			}
			else
			{
				if (gfDepth >= -40.0f)
				{
					gfDepth = gfDepth - 0.02f;
				}
			}
			break;
		case 'X': // Zoom Out
		...
		case 'c':
			if (switchCameraToMovement)
			{
				switchCameraToMovement = FALSE;
			}
			else
			{
				switchCameraToMovement = TRUE;
			}
			break;
		case 'V':
		case 'v':
...
int initialize(void)
{
	...
	if (result != 0)
	{
		fprintf(gpFILE, "user Defined initializeScene() Siddhant failed.\n");
		return(-6);
	}

	result = SceneInitialize();
	if (result != 0)
	{
		fprintf(gpFILE, "user Defined initializeScene() Hrishikesh failed.\n");
		return(-7);
	}
